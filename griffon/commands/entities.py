...
from griffon.autocomplete.product_streams import ofuris, product_stream_names
...
def get_product_stream_ofuris(ctx, param, incomplete):
    return [k for k in ofuris if k.startswith(incomplete)]


def get_product_stream_names(ctx, param, incomplete):
    return [k for k in product_stream_names if k.startswith(incomplete)]
